...
nose_landmarks_indices = list(range(160, 200)) # black
        ...
        for face_landmarks in results.multi_face_landmarks:
            for i, landmark in enumerate(face_landmarks.landmark):
                if i in eye_landmarks_indices:
                    ...
                    cv2.putText(frame, str(i), (x, y), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0), 1)
                elif i in nose_landmarks_indices:
                    nose_landmarks.append(landmark)
                    # 랜드마크 인덱스 화면에 표시
                    x, y = int(landmark.x * frame.shape[1]), int(landmark.y * frame.shape[0])
                    cv2.putText(frame, str(i), (x, y), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (20, 0, 0), 1)
                elif i in mouth_landmarks_indices:
                    mouth_landmarks.append(landmark)
                    # 랜드마크 인덱스 화면에 표시
                    x, y = int(landmark.x * frame.shape[1]), int(landmark.y * frame.shape[0])
                    cv2.putText(frame, str(i), (x, y), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255), 1)
